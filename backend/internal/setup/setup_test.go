...
func TestGetMigrationTimeout(t *testing.T) {
	...
	t.Run("uses env override when valid", func(t *testing.T) {
		t.Setenv(migrationTimeoutEnv, "900")

		if got := getMigrationTimeout(); got != 15*time.Minute {
			t.Fatalf("getMigrationTimeout()=%s, want %s", got, 15*time.Minute)
		}
	})

	t.Run("falls back when env invalid", func(t *testing.T) {
		t.Setenv(migrationTimeoutEnv, "abc")
