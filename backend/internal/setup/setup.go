...
const (
	...
	migrationTimeoutEnv        = "SETUP_DATABASE_MIGRATION_TIMEOUT_SECONDS"
)
...
func initializeDatabase(cfg *SetupConfig) error {
	...
	timeout := getMigrationTimeout()
	logger.LegacyPrintf("setup", "Using database migration timeout: %s", timeout)
	migrationCtx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
...
func getMigrationTimeout() time.Duration {
	...
	if err != nil || seconds <= 0 {
		logger.LegacyPrintf(
			"setup",
			"%s must be a positive integer number of seconds, got %q; using default %s",
			migrationTimeoutEnv,
			raw,
