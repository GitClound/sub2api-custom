export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      cliproxyAuthImportSuccess: '导入完成：新建 {account_created}，替换 {account_replaced}，跳过 {account_skipped}',
      ...
      cliproxyAuthImportResultSummary: '代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；账号创建 {account_created}，替换 {account_replaced}，跳过 {account_skipped}，失败 {account_failed}',
      cliproxyAuthDuplicateTitle: '发现重复的 CLIProxyAPI Auth',
      cliproxyAuthDuplicateMessage: '检测到 {count} 个导入账号与现有账号重复。你可以选择保留现有账号并跳过重复项，或直接用导入内容替换现有账号。',
      cliproxyAuthDuplicateSourceFile: '来源文件：{file}',
      cliproxyAuthDuplicateExisting: '现有账号：{name}（ID: {id}），匹配依据：{matchedBy}',
      cliproxyAuthDuplicateKeepExisting: '跳过重复项',
      cliproxyAuthDuplicateReplaceExisting: '替换现有账号',
      ...
      openai: {
        baseUrlHint: '留空使用官方 OpenAI API，也可填写 OpenAI 兼容提供商的 Base URL。',
        apiKeyHint: '您的 OpenAI API Key，或兼容提供商分配的 API Key。',
        compatTitle: 'OpenAI 兼容提供商（可选）',
        compatDescription: '当前 OpenAI API Key 账号已支持 OpenAI-compatible 提供商。将 Base URL 指向兼容端点后，可在此补充供应商名称和自定义请求头。',
        providerName: '供应商名称',
        providerNamePlaceholder: '例如：OpenRouter、Together、Groq',
        providerNameHint: '仅用于管理端识别当前兼容提供商，不参与请求签名。',
        customHeaders: '自定义请求头',
        customHeadersPlaceholder: '{\n  \"HTTP-Referer\": \"https://example.com\",\n  \"X-Title\": \"sub2api\"\n}',
        customHeadersHint: '请输入 JSON 对象。常用于 OpenRouter 等兼容提供商的额外请求头。',
        customHeadersInvalid: '自定义请求头必须是合法 JSON 对象，且值需为字符串。',
